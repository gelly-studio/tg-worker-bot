...
🚀 部署步骤
1. 准备工作
Telegram Bot: 与 @BotFather 对话获取 TG_BOT_TOKEN。
Chat ID: 获取你要发送消息的频道或群组 ID (TG_CHAT_ID)。
Cloudflare Turnstile: 在 Cloudflare Dashboard 注册一个站点，获取 SITE KEY 和 SECRET KEY。
Cloudflare D1: 创建一个新的 D1 数据库，获取 Database ID。
2. 创建 Worker
在 Cloudflare Dashboard 创建一个新的 Worker。
将本项目的 index.js 代码完整粘贴到编辑器中并保存。
绑定 D1 数据库：
进入 Worker 的 Settings -> Variables and Secrets。
在 D1 database bindings 区域，添加 Binding，变量名填写 DATABASE，选择刚才创建的 D1 数据库。
3. 配置环境变量
在 Settings -> Variables and Secrets 的 Environment variables 区域添加以下变量：
...
DOMAIN	域名 (选填，通常留空)	-
4. 部署完成
点击 "Deploy & Edit"，记录下你的 Worker 访问地址（如 https://tg-worker.your-name.workers.dev）。
...
人机验证：勾选 "我是机器人" 复选框完成验证（必须完成勾选，无法使用验证码通过）。
...
3. 后台图库管理
点击右上角用户头像下拉菜单中的 后台管理，或直接访问 /你的后台路径 进入管理页。
...
选中图片后，点击顶部红色 删除 按钮。
这将同时执行两个操作：
从 Telegram 频道中删除对应的消息。
从 D1 数据库中删除对应记录。
删除操作不可恢复。
🔐 安全建议
请务必修改默认的 USERNAME 和 PASSWORD。
将 Worker 的自定义域名设为 HTTPS，并防止第三方爬虫访问（可以通过 Turnstile 提供基本防护）。
建议将 GitHub 仓库设置为 Private (私有)，以免泄露你的架构逻辑。
注：
本程序代码完全在浏览器端和 Cloudflare 边缘节点运行，不依赖任何后端服务器。
